-- Partner-level install EFFICIENCY, June vs July, by MBG enrollment category.
-- Efficiency (per CSP) = installs / executor-assigned tasks.
--   task   = EXECUTOR_ID IS NOT NULL            (a job actually handed to the CSP)
--   install= INSTALLATION_COMPLETED_AT IS NOT NULL
-- 60%+ = a CSP whose efficiency >= 0.60. Filtered to >=3 executor-assigned tasks
-- (drops 1-2 task 100%/0% flukes). July bucketed 01 Jul -> CURRENT_DATE-2 so the
-- window is install-matured (installs resolve ~97% within 3 days), June = full month.
-- Category (via CSP_ACCOUNT partner_id->csp_id, from the belief-break enrolment audit):
--   enrolled = audit done / ENROLLED (461 csp) · eligible = offered but not enrolled
--   (108 csp) · nonmbg = everyone else. Scope caveat: TAS = new F1 OS app only.
WITH cand AS (
  SELECT CSP_ID,
    CASE WHEN DATE_TRUNC('month', CREATED_AT) = '2026-06-01' THEN 'june'
         WHEN CREATED_AT >= '2026-07-01'
              AND CREATED_AT < DATEADD('day', -2, CURRENT_DATE) THEN 'july'
    END AS period,
    COUNT(CASE WHEN EXECUTOR_ID IS NOT NULL THEN 1 END)              AS tasks,
    COUNT(CASE WHEN INSTALLATION_COMPLETED_AT IS NOT NULL THEN 1 END) AS installs
...
cat AS (
  SELECT CSP_ID, period, tasks, installs,
    installs / NULLIF(tasks, 0)::float AS eff,
  ...
  FROM cand
  WHERE period IS NOT NULL AND tasks >= 3
)
SELECT period AS PERIOD, category AS CATEGORY,
  COUNT(*)                                      AS PARTNERS,
  COUNT(CASE WHEN eff >= 0.60 THEN 1 END)        AS GE60,
  ROUND(100.0 * COUNT(CASE WHEN eff >= 0.60 THEN 1 END) / COUNT(*), 1) AS PCT_GE60,
  SUM(tasks)                                     AS TASKS,
