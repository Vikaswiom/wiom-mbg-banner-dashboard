...
-- pre-program baseline for the same partners. Buckets: enrolled = audit done (461 csp) ·
-- eligible = offered, not enrolled (108) · nonmbg = everyone else. Matches vikaswiom.github.io/
-- wiom-mbg-kamai-kavach/data.json for July.
WITH base AS (
...
cat AS (
  ...
  FROM per_csp
  WHERE period IS NOT NULL AND denom > 0    -- only partners with measurable % this month
)
SELECT period AS PERIOD, category AS CATEGORY,
  COUNT(*)                                   AS PARTNERS,   -- partners with leads (denom>0)
  SUM(denom)                                 AS LEADS,      -- resolved slot-confirmed leads
  SUM(installs)                              AS INSTALLS,
  ROUND(100.0 * SUM(installs) / NULLIF(SUM(denom), 0), 1) AS AGG_EFF,
  COUNT_IF(eff > 0.60)                        AS SECURED,   -- cleared the 60% gate
